...
class GlslShader(
    ...
    override fun createRenderer(surface: Surface, renderContext: RenderContext): Renderer {
        ...
        val pooledRenderer = renderContext.registerPooled(poolKey) { PooledRenderer(program) }
    ...
    override fun createRenderer(surface: Surface): Renderer {
        val glslRenderer = GlslBase.manager.createRenderer(program)
    ...
    class PooledRenderer(program: Program) : baaahs.PooledRenderer {
        val glslRenderer = GlslBase.manager.createRenderer(program)
