...
void glue_callListener(void *pArg, uint32_t pArg2) {
    ((DHTSensor*)pArg)->_callListener(pArg2);
}
...
//    if (m_Expect == ClearLine) {
        ...
        _sendReadStart();
//    } else {
//        ESP_LOGI(TAG, "Refusing to do a read because state=%d", m_Expect);
//    }
}
...
void
DHTSensor::_callListener(uint32_t edgeIx) {
...
//    xTimerStop(m_hEdgeTimeout, 0);
//
//    if (millis) {
//        TickType_t ticks = pdMS_TO_TICKS(millis);
//        if (!ticks) ticks = 1;
//        ESP_LOGD(TAG,"_setTimeout ticks=%d", ticks);
//        xTimerChangePeriod(m_hEdgeTimeout, ticks, 0);
//        xTimerStart(m_hEdgeTimeout, 0);
//    } else {
//        ESP_LOGD(TAG,"_setTimeout 0");
//    }
}
